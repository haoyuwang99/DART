#!/usr/bin/env python3
"""Unified scorer: one pass computes both state (h_i) and transition (h_i-h_{i-1})
per tool-result context, and reports per-suite + mean leave-one-suite-out AUROC
for BOTH forms. Same context set + encoding for a fair state-vs-transition read.

Usage: score_final.py <model_id> <traces.jsonl>
"""
...
def main():
    ...
    for line in open(TRACES):
        ...
        for i, m in enumerate(msgs):
            ...
            h_i = hidden(build_messages(msgs[:i+1])); h_prev = hidden(build_messages(msgs[:i]))
            data.append((r["suite"], 1 if MARK in m["text"] else 0, h_i, h_i - h_prev))
    suites = sorted({d[0] for d in data})
    print(f"model={MODEL} contexts={len(data)} inj={sum(d[1] for d in data)}")
    ...
    def loso(repidx):
        ...
        for ho in suites:
            ...
            if len({data[i][1] for i in te}) < 2: per[ho] = float("nan"); continue
            ...
            d = X[y==1].mean(0) - X[y==0].mean(0); d /= np.linalg.norm(d)+1e-9
            per[ho] = auroc([data[i][repidx][bestL]@d for i in te], [data[i][1] for i in te])
        return per
    for name, idx in [("STATE", 2), ("TRANSITION", 3)]:
        per = loso(idx); mean = np.nanmean(list(per.values()))
        cols = " ".join(f"{s}={per[s]:.2f}" for s in suites)
        print(f"  {name:11} {cols} | MEAN={mean:.3f}")
